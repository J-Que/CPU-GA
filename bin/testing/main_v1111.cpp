...
vector<vector<int>> parentRankings;     // ranks the best performing offspring and parents
vector<vector<int>> offspringRankings;  // ranks the best performing offspring and parents 
...
void brief() {
    ...
    cout << "\n   Nodes: " << setw(25) << N << " Vehicles: " << setw(22) << trucks << " Capacity: " << setw(22) << capacity << " Best Known Cost: " << setw(22) << best << " Optimal: " << optimality;
...
void summary() {
    ...
    cout << "\n    " <<setw(32)<< gen <<setw(33)<< score <<setw(33)<< (score - best)/best <<setw(33)<< t/gen << t;
}
...
// save the results
void save(bool saveGenes, char* run, char* selectionRate) {
    ofstream results ("../../results/" + CVRP + "_v1111_" + run + "_" + selectionRate + "_report.json");
    auto t = chrono::duration_cast<chrono::milliseconds>(chrono::high_resolution_clock::now() - start).count()/1000.0;
    int solution;
    for (int m = 0; m < M; m++) {if (population[m][0] == score) {solution = m;}}
        ...
        if (pheno[gene] == depot) {gene++;}        // if the current gene is a depot, then pass over it
            ...
            while (improved) {
                ...
                for (int i = start; i < end - 1; i++) {
                    ...
                    vector<int> node3 = decode(pheno[i]);
                    vector<int> node1 = decode(pheno[i - 1]);

                    for (int j = i + 1; j < end; j++) {

                        vector<int> node2 = decode(pheno[j]);
                        vector<int> node4 = decode(pheno[j + 1]);

                        // get the change in distance if the two opt is performed (subtract the arcs and add the new ones)
                        float deltaDistance = 0;
                        deltaDistance -= sqrt(pow(node3[0] - node1[0], 2) + pow(node3[1] - node1[1], 2));
                        deltaDistance -= sqrt(pow(node4[0] - node2[0], 2) + pow(node4[1] - node2[1], 2));
                        deltaDistance += sqrt(pow(node2[0] - node1[0], 2) + pow(node2[1] - node1[1], 2));
                        deltaDistance += sqrt(pow(node4[0] - node3[0], 2) + pow(node4[1] - node3[1], 2));

                        // if the new distance improves then the new best distance is recorded
                        if (deltaDistance < 0) {
                            // perform the actual two opt
                            for (int a = 0; 2*a <= j - i; a++) {swap(pheno[i + a], pheno[j - a]);}
                            improved = true;
                            goto REPEAT;
                        } 
                    }
...
void crossover(int m, int q, int gen, vector<int> &pairs) {
    ...
    if (chance <= CROSSOVER_RATE) {
        ...
        // get randomize locations of the cut
        int cut = rand() % (2*N - 5) + 3;

        // add the first part of each parent to the respective child (upto cut 1)
        copy(parent1.begin(), parent1.begin() + cut, back_inserter(offspring1));
        copy(parent2.begin(), parent2.begin() + cut, back_inserter(offspring2));

        // add the second part of each parent to the respective child (between cut 1 and cut 2)
        copy(parent1.begin() + cut, parent1.end(), back_inserter(offspring1));
        copy(parent2.begin() + cut, parent2.end(), back_inserter(offspring2));
...
void mutate(int q) {
    ...
    if (chance <= MUTATION_RATE) {
        int cut1 = (rand() % (2*N - 5)) + 3;                // find a random location for cut
        int cut2 = (rand() % (2*N - cut1 -2)) + cut1 + 1;	// find a random location for second cut
...
void select() {
    int selectedParents = (int) (ELITISM_RATE * population.size());
    int selectedOffspring = population.size() - selectedParents;

    // Get the order and fitness of the offspring population
    for (int m = 0; m < offspring.size(); m++) {offspringRankings.push_back(vector<int> {m, offspring[m][0]});}

    // Sort the population by fitness
    sort(offspringRankings.begin(), offspringRankings.end(), [](const vector<int> &a, const vector<int> &b) {return a[1] < b[1];});

    // Get the order and fitness of the parent population
    for (int m = 0; m < population.size(); m++) {parentRankings.push_back(vector<int> {m, population[m][0]});}
 
    // Sort the population by fitness
    sort(parentRankings.begin(), parentRankings.end(), [](const vector<int> &a, const vector<int> &b) {return a[1] < b[1];});

    // skip over the amout the top performing parents on the rankings vector
    for (int m = selectedParents; m < parentRankings.size(); m++) {population[parentRankings[m][0]] = offspring[offspringRankings[m - selectedParents][0]];}

    // clear the vectors for the next generation
    parentRankings.clear();
    offspringRankings.clear();
    offspring.clear();
}
